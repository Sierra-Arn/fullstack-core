...
def generate_sql_script(output_dir: Path) -> Path:
    ...
    sql_content = (
        ...
        f"GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO \"{env_vars['POSTGRES_USER_NAME']}\";\n"
        f"ALTER DEFAULT PRIVILEGES IN SCHEMA public\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--output-dir",
        type=Path,
        default=Path.cwd() / "postgres" / "init",
        help="Directory to save the generated SQL file. Default is ./postgres/init.",
    )
