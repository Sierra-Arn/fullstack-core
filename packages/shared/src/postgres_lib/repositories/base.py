...
# packages/shared/src/postgres_lib/repositories/base.py
...
"""
Generic type variable representing any SQLAlchemy ORM model in the application.

Used to parameterize repository operations while preserving static type safety
across different entity types. The bound constraint restricts valid
substitutions to subclasses of the declarative base, which every model inherits
from.
"""
...
class BaseRepository(ABC):
    ...
    connection-agnostic API surface.

    Attributes
    ----------
    model_class : ClassVar[type[ModelType]]
        SQLAlchemy ORM model class managed by the repository, bounded by
        IdModel. Must be explicitly overridden in each concrete subclass with
        a concrete model type to enable strict static type checking for
        query and persistence operations.

    Notes
    -----
    All data access methods are classmethods that accept a session as the first
    argument. Transaction lifecycle, including commit, rollback, and close, is managed
    externally by the caller, never inside the repository. Creation and update
    payloads are plain dictionaries, typically derived from validated request
    shapes. Update dictionaries must contain only mutable fields; primary keys
    and server-generated fields such as created_at are never overwritten.
    Internal methods call session.flush() and session.refresh() as needed to
    synchronize ORM state with the database after writes.
    """
    ...
    async def update_by_id(
        ...
        """
        Update a record by its primary key with new field values.

        Only fields explicitly provided in obj_data are modified.
        Primary key and server-generated fields such as created_at
        are never overwritten regardless of what obj_data contains.
        ...
        -------
        ModelType or None
            Updated ORM instance with refreshed field values,
            or None if no record exists with the given primary key.
        """
    ...
    async def count(
        ...
        Used to calculate pagination metadata without fetching the full
        result set. Executes a single COUNT query against the model's
        table.
        ...
        -------
        int
            Total number of records currently in the table.
        """
        stmt = select(func.count()).select_from(cls.model_class)
        result = await session.execute(stmt)
